from dataset_ORB_arrow import BasicDataset
from linear_network_ORB_arrow_alt import BasicNetwork, train_loop, test_loop
...
if __name__ == "__main__":
    ...
    new_name = "ORB 100k, 10000 epochs"
    wandb.run.name = new_name
    wandb.run.save()

    dataset = BasicDataset("../../data/training_100000pix_200m.csv")
    torch.save(dataset, "new_data/training_100000_ORB_arrow_pix_dataset.pt")
    print("data saved")
    '''dataset = torch.load("new_data/training_100000_ORB_arrow_pix_dataset.pt")
    print("data loaded")'''
    ...
    try:
        for epoch in range(epochs):
            ...
            testing_dicts, test_error, te_key = test_loop(valid_loader, model, loss_fn, device, epoch, batch_size, will_save, te_key)
            if will_save:
