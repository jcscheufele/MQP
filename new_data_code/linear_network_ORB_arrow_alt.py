...
def train_loop(dataloader, model, loss_fn, optimizer, device, epoch, bs, will_save, key):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        loss = loss_fn(pred, y[:, 2].to(device))
        ...
        if will_save and (batch % 10 == 0):
            ...
            for idx in range:
                img1 = X[idx][:-3].numpy().reshape((240,426))
                loc = X[idx][-3:]
                save = {"Train Key": key, "Sample Epoch":epoch,"Sample Training Loss":loss,
                "Sample Training First Image": wandb.Image(img1), 
                ...
                "Sample Training Truth Alt": y[idx][2].item()}
                ret.append(save)
                key+=1
      ...
      for batch, (X, y) in enumerate(dataloader):
        ...
        if will_save and (batch % 5 == 0):
            ...
            for idx in range:
                ...
                "Sample Testing First Image": wandb.Image(img1), 
                ...
                "Sample Testing Truth Alt": y[idx][2].item()}
                ret.append(save)
                key+=1
            row = f"{epoch} [ {batch}/{batches} ] Loss: {loss} SAVED\n"
            print(row) 
